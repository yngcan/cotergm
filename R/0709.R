...
netlist <- list(net1,formed.net.p,formed.net.m,dissolved.net.p,dissolved.net.m,net2)
netlist
...
statlist <- lapply(netlist,function(x){
            s <- summary(x~edges+nodematch("alcohol",diff=TRUE))
            ss <- c(s,s[1]-s[2]-s[3])
            names(ss) <- c(names(s),"01")
            ss
        }
)

names(statlist) <- c("net1","F+","F-","D+","D-","net2")
