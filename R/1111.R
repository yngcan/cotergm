...
add.edges(net1,tail=c(4,1),head=c(3,4))
net1%v%"status" <- c(2,1,1,2)

net2 <- network.initialize(4,directed=FALSE)
add.edges(net2,tail=c(2,4),head=c(1,3))
net2%v%"status" <- c(2,2,1,1)
...
# influence - 
summary(DM~edges+plus+nodematch_cotergm("status",diff=TRUE,y0nodal=2,keep=1),MHproposal=MHproposal)

# influence + 
summary(DP~edges+plus+nodematch_cotergm("status",diff=TRUE,y0nodal=2,keep=2),MHproposal=MHproposal)

# selection +
summary(FP~edges+plus+nodematch_cotergm("status",diff=TRUE,y0edge=2,keep=1),MHproposal=MHproposal)

# selection -
summary(FM~edges+plus+nodematch_cotergm("status",diff=TRUE,y0edge=2,keep=2),MHproposal=MHproposal)
