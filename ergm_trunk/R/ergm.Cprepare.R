...
ergm.Cprepare <- function(nw, m, response=NULL)
{
	...
	e<-as.edgelist(nw,attrname=response) # Ensures that for undirected networks, tail<heWad.
	if(length(e)==0){
		Clist$nedges<-0
		Clist$tails<-NULL
		Clist$heads<-NULL
	...
	Clist$nodalstatus <- rep(0,n)
	
	if(!any(is.null(get.vertex.attribute(nw,"status"))) & length(get.vertex.attribute(nw,"status"))==n){
		if(all(get.vertex.attribute(nw,"status") %in% c("-","+")))
			Clist$nodalstatus <- match(get.vertex.attribute(nw,"status"),c("-","+")) 
		else if (all(get.vertex.attribute(nw,"status") %in% c(1,2))){
			Clist$nodalstatus <- get.vertex.attribute(nw,"status")
		}}
#	cat("Clist$nodalstatus",Clist$nodalstatus,"length=",length(get.vertex.attribute(nw,"status")),"\n")
	Clist
}
