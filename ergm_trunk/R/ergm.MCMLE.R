...
ergm.MCMLE <- function(init, nw, model,
  ...
  for(iteration in 1:control$MCMLE.maxit){
    ...
    repeat{ # Keep trying until effective sample size is
        ...
        if(sequential) {
            nw <- nw.returned
            statshift <- summary(model$formula, basis=nw, response=response, MHproposal=MHproposal) - model$target.stats
            
            if(obs){
                nw.obs <- nw.obs.returned
                statshift.obs <- summary(model$formula, basis=nw.obs, response=response, MHproposal=MHproposal) - model$target.stats
            }      
        }
