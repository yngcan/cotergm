...
#  model.initial <- extremecheck$model; control$init <- extremecheck$init
  ...
  model.initial$nw.stats <- summary(model.initial$formula, response=response, initialfit=control$init.method=="MPLE",MHproposal=MHproposal)
...
#    initialfit$drop <- if(control$drop) extremecheck$extremeval.theta
  ...
  init[sapply(init,function(x)abs(x)>5)] <- 1
  init[is.na(init)] <- 1
  
#  init <-replicate(0,length(model.initial$coef.names))
...
#  model <- extremecheck$model; init <- extremecheck$init
  ...
  model$nw.stats <- summary(model$formula, response=response,MHproposal=MHproposal)
