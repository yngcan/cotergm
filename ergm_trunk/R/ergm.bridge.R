...
ergm.bridge.preproc<-function(object, basis, response, MHproposal=NULL){
...
ergm.bridge.llr<-function(object, response=NULL, constraints=~., from, to, basis=NULL, verbose=FALSE, ..., llronly=FALSE, control=control.ergm.bridge(),  MHproposal=NULL){
  ...
  }else stats.obs<-matrix(summary(form,response=response,MHproposal=MHproposal),control$nsteps,m$etamap$etalength,byrow=TRUE)  
  ...
  for(i in seq_len(control$nsteps)){
    ...
    nw.state<-simulate(form, coef=theta, nsim=1, response=response, constraints=constraints, statsonly=FALSE, verbose=max(verbose-1,0),
                       control=control.simulate.formula(MCMC.burnin=if(i==1) control$MCMC.burnin else ceiling(control$MCMC.burnin/sqrt(control$nsteps)),
                         ...
                         MCMC.packagenames=control$MCMC.packagenames), ...,MHproposal=MHproposal)
    ...
    stats[i,]<-apply(simulate(form, coef=theta, response=response, constraints=constraints, statsonly=TRUE, verbose=max(verbose-1,0),
                              ...
                              nsim=ceiling(control$MCMC.samplesize/control$nsteps), ...,MHproposal=MHproposal),2,mean)
    ...
    if(network.naedgecount(nw)){
      ...
      stats.obs[i,]<-apply(simulate(form.obs, coef=theta, response=response, constraints=constraints.obs, statsonly=TRUE, verbose=max(verbose-1,0),
                                ...
                                nsim=ceiling(control$obs.MCMC.samplesize/control$nsteps), ...,MHproposal=MHproposal),2,mean)
    }
  }
...
ergm.bridge.dindstart.llk<-function(object, response=NULL, constraints=~., coef, dind=NULL, coef.dind=NULL,  basis=NULL, ..., llkonly=TRUE, control=control.ergm.bridge(),MHproposal=NULL){
  check.control.class("ergm.bridge")
  ...
  br<-ergm.bridge.llr(form.aug, response=response, constraints=constraints, from=from, to=to, basis=basis, control=control,MHproposal=MHproposal)
