...
"obs.MCMC.burnin", "obs.MCMC.interval", "obs.MCMC.samplesize","warn.dyads","MPLE.max.dyad.types")
  ...
  out<-with(object,
            {
              ...
              if(estimate=="MPLE"
                ...
                if(is.dyad.independent(object$constrained, object$constrained.obs)
                   && is.null(object$response))
                  ergm.bridge.dindstart.llk(formula,reference=reference,constraints=constraints,coef=coef(object),control=loglik.control,llkonly=FALSE,MHproposal=object$MHproposal,...)
                else
                  ## If valued or has dyad-dependent constraint, compute a path sample from reference measure.
                  ergm.bridge.0.llk(formula,response=object$response,reference=reference,constraints=constraints,coef=coef(object),control=loglik.control,llkonly=FALSE,MHproposal=object$MHproposal,...)
            }
            )
