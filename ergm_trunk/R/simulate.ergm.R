...
simulate.ergm <- function(object, nsim=1, seed=NULL, 
  ...
  simulate.formula(object$formula, nsim=nsim, coef=coef, response=response, reference=reference,
                   ...
                   monitor=monitor,
                   control=control, verbose=verbose, seed=seed,MHproposal=MHproposal, ...)
}
...
simulate.formula <- function(object, nsim=1, seed=NULL,
  ...
  curstats<-summary(form,response=response, MHproposal=MHproposal)
  ...
  if(sequential && statsonly){
    ...
    z <- ergm.getMCMCsample(nw, m, MHproposal=MHproposal, eta0, control, verbose=verbose, response=response)
