...
summary.statistics <- function(object, ..., basis=NULL,MHproposal=NULL) {
  UseMethod("summary.statistics")
}


summary.formula <- function(object, ...,MHproposal=NULL){
...
summary.statistics.formula <- function(object, ..., basis=NULL,MHproposal=NULL){
  summary.statistics.network(object, ..., basis=basis,MHproposal=MHproposal)
}



summary.statistics.ergm <- function(object, ..., basis=NULL,MHproposal=NULL)
{
  summary.statistics.network(object$formula, ..., basis=basis,MHproposal=MHproposal)
}

summary.statistics.network.list <- function(object, response=NULL, ..., basis=NULL,MHproposal=NULL){
  if(!is.null(basis)){
  ...
  out<-lapply(nwl, function(nw) summary.statistics.network(object, response=response, ..., basis=nw,MHproposal=MHproposal))
  do.call(rbind,out)
}

summary.statistics.default <-
summary.statistics.matrix <- 
summary.statistics.network <- function(object, response=NULL,...,basis=NULL,MHproposal=NULL){
  if(is.network(basis)){
    nw <- basis
  ...
  gs <- ergm.getglobalstats(nw, m, response=response, MHproposal=MHproposal)
  gs
}
