...
void MCMC_wrapper(int *dnumnets, int *nedges,
	...
	y0[0]=NetworkInitialize(y0tails, y0heads, y0nedges[0],
				n_nodes, directed_flag, bip, 0, 0, NULL, y0nodalstatus);
...
//	Rprintf("I am here, where is the bug \n");
   ...
   Burn in step.
	...
	if (samplesize>1){
		...
		for (i=1; i < samplesize; i++){
			...
			/* deal with when all nodes are of the same status */
			/*if(nwp->nplus >= statusmax | nwp->nplus <= (nwp->nnodes-statusmax)){
				samplesize += 1;
				continue;
			}
			*/
	...
	for(unsigned int step=0; step < nsteps; step++) {
       ...
       But we'll do it in log space instead.  */
		...
		if (cutoff >= 0.0 || log(unif_rand()) < cutoff) {
			...
			if(MHp->togglenode[0] == 0){/*toggle dyad */
				...
				}} else { /*toggle nodal status*/
					for(unsigned int i=0; i < MHp->ntoggles; i++){
						ToggleNode(MHp->togglenode[i],nwp);}
				}


			for (unsigned int j = 0; j < nwp->nnodes; j++){
				if(fVerbose>=3){
						Rprintf("%f\t",nwp->nodalstatus[j]);
					}
			}
