...
/*      Rprintf("fn: %s\n",fn); */
      ...
      thisterm->d_func = 
	(void (*)(int, Vertex*, Vertex*, Vertex*, ModelTerm*, Network*, Network*))
...
/*      Rprintf("l %d offset %d thisterm %d\n",l,offset,thisterm->nstats); */
  ...
  m->nodalstatus = (double *) malloc(sizeof(double) * n_nodes);

  for(i=0;i< n_nodes;i++)
	  m->nodalstatus[i] = nodalstatus[i];
...
*/
void ChangeStats(unsigned int ntoggles, Vertex *toggletail, Vertex *togglehead, Vertex *togglenode,
  ...
  for (unsigned int i=0; i < m->n_terms; i++){
    ...
    (*(mtp->d_func))(ntoggles, toggletail, togglehead, togglenode,
