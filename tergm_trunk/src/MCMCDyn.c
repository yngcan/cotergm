...
void MCMCDyn_init_common(int *tails, int *heads, int time, int *lasttoggle, int n_edges,
  ...
  nodalstatus = (double *)malloc( nw->nnodes * sizeof(double));

  if(F_nterms) *F_m=ModelInitialize(F_funnames, F_sonames, &F_inputs, F_nterms, n_nodes, nodalstatus); else if(F_m && *F_m) *F_m=NULL;
  if(D_nterms) *D_m=ModelInitialize(D_funnames, D_sonames, &D_inputs, D_nterms, n_nodes, nodalstatus); else if(D_m && *D_m) *D_m=NULL;
  if(M_nterms) *M_m=ModelInitialize(M_funnames, M_sonames, &M_inputs, M_nterms, n_nodes, nodalstatus); else if(M_m && *M_m) *M_m=NULL;

  nw[0]=NetworkInitialize(tails, heads, n_edges, 
                          n_nodes, dflag, bipartite, 1, time, lasttoggle, nodalstatus);

  if(F_MH || D_MH) nw[1]=NetworkInitialize(NULL, NULL, 0,
					   n_nodes, dflag, bipartite, 0, 0, NULL, nodalstatus);
...
*/
 void MCMCDyn1Step_sample(MHproposal *MH,
  ...
  for(unsigned int finished = 0, extrasteps = 0; step < max_MH_interval && finished < extrasteps+1; step++) {
    ...
    ChangeStats(MH->ntoggles, MH->toggletail, MH->togglehead,togglenode, nwp, m, nwp);
...
void MCMCDyn1Step_advance(unsigned int ntoggles,
  ...
  if(F_stats){
    ChangeStats(ntoggles,difftail,diffhead, togglenode,nwp,F_m,nwp);
    for (unsigned int i = 0; i < F_m->n_stats; i++)
      F_stats[i] += F_m->workspace[i];
  }
  
  if(D_stats){
    ChangeStats(ntoggles,difftail,diffhead, togglenode,nwp,D_m,nwp);
    for (unsigned int i = 0; i < D_m->n_stats; i++)
      D_stats[i] += D_m->workspace[i];
  }
  
  if(M_stats){
    ChangeStats(ntoggles,difftail,diffhead, togglenode,nwp,M_m,nwp);
