...
{  
  ...
  BD_COND_LOOP({
    /* Keep trying dyads until a one that is not an edge in the reference network is found. */
    /* Generate. */
      GetRandDyad(Mtail, Mhead, nwp);  //save to Mtail and Mhead, not read from
    }, !dEdgeListSearch(Mtail[0],Mhead[0],MHp->inputs), 2);
}
...
{  
  ...
  if(MHp->ntoggles == 0) { /* Initialize */
    ...
    discord = NetworkInitializeD(MHp->inputs+1, MHp->inputs+1+nedges0, nedges0, nnodes, nwp->directed_flag, nwp->bipartite, 0, 0, NULL,nwp->nodalstatus);
   // now discord equals to reference graph
    for(Edge i=0; i<nwp->nedges; i++){
      FindithEdge(&tail, &head, i+1, nwp); // look at proposal network
      ToggleEdge(tail,head, &discord); // change the discord according to proposal network
    }
